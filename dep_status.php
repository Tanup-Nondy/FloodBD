...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-content-wrapper">
			...
			<center>
			<div>
			<center><h1><u>Money Status</u></h1></center>
                <?php
					$cn=mysql_connect("localhost","root","");
					mysql_select_db("flood",$cn);
					$total=0;
					$tgoods=0;
					$col="SELECT * FROM money";
					$chk=mysql_query($col,$cn);
					if($chk){
						while($row=mysql_fetch_array($chk)){
								$total=$total+$row['amount'];
						}
						echo '<h2>'."Total Cash deposited yet: ".$total."$".'</h2>'.'</br>';
					}
					echo '<center><h1><u>Goods Status</u></h1></center>';
					$col="SELECT * FROM goods";
					$chk=mysql_query($col,$cn);
					if($chk){
						while($row=mysql_fetch_array($chk)){
								$tgoods=$tgoods+$row['goods_weight'];
						}
						echo '<h2>'."Total Goods deposited yet: ".$tgoods."Kg".'</h2>'.'</br>';
					}
				?>  
				</div>
				</center>
